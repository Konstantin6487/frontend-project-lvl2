...
const typesRenders = {
  added: ({ value }, path) => `Property '${stringifyNodePath(path)}' was added with value: ${stringifyNodeValue(value)}`,
  removed: (_, path) => `Property '${stringifyNodePath(path)}' was removed`,
  nested: ({ children }, path, fn) => fn(children, path),
  unchanged: () => null,
  changed: ({ newValue, originalValue }, path) => `Property '${stringifyNodePath(path)}' was updated. From ${stringifyNodeValue(originalValue)} to ${stringifyNodeValue(newValue)}`,
};
